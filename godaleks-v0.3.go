// v2.2. Read and write score file
package main
...
func drawboard() { // Draw the board again after changing something
	...
	fmt.Printf("Level: %d Score: %d (High Score: %s)\n", level, score, highscore)
}
...
func writehighscore() {
	mydata := []byte(strconv.Itoa(score))

	// the WriteFile method returns an error if unsuccessful
	err := ioutil.WriteFile("highscore.txt", mydata, 0777)
	// handle this error
	if err != nil {
		// print it out
		...
		if dalekscoord[i][2] == 0 { // Not dead yet. OK to move
			...
			if dalekonwho() == 1 { // Dalek landed on Doctor Who.
				...
				if lives == 0 {
					...
					drawboard()
					highscoreint, err := strconv.Atoi(highscore)
					if err != nil {
						panic(err)
					}
...
func main() {
	...
	// This read/write file code is really weird. I am sure there is an easier way to read/write a single int from a file.
	// I'm working off random Internet examples. I had to change my highscore variable to a string to make the reading and
	// printiung easier.

	// Check to see if the highscore.txt exists, if not create
	if _, err := os.Stat("highscore.txt"); err == nil { // file exists
		filescore, fileerr := ioutil.ReadFile("highscore.txt")
		if fileerr != nil {
			fmt.Println("Could not open highscore.txt")
		} else {
			//filescoreint := binary.BigEndian.Uint16(filescore)
			filescorestring := string(filescore)
			highscore = filescorestring
		}
	} else { // file does not exist
		err := ioutil.WriteFile("highscore.txt", []byte("0"), 0777)
		// handle this error
		if err != nil {
			// print it out
			fmt.Println("highscore.txt creation error", err)
		}
	}
